...
import { suggestedCounter } from "@/lib/engine";
...
export function CounterOfferButton({
  ...
  if (variant === "dark") {
    return (
      <button onClick={start} className="flex w-full items-center justify-center gap-2 rounded-full bg-white/15 py-3 text-sm font-medium text-white">
  ...
  if (variant === "text") {
    return (
      <button onClick={start} className="flex items-center gap-1.5 text-xs font-medium text-ink-950 hover:underline">
        <Handshake className="h-3.5 w-3.5" /> {label}
      </button>
    );
  }

  return (
    <Button size="sm" variant={variant} onClick={start}>
      <Handshake className="h-3.5 w-3.5" /> {label}
    </Button>
  );
}
