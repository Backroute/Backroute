...
import { addActivity, claimMark, logChannel, save, type CarrierContext } from "./db";
...
export const VOICEMAIL = (ctx: CarrierContext, load: Load) =>
  `Hi, this is the AI dispatcher for ${ctx.carrier.name}, about your load ${load.referenceNumber}. We'd like to book it at ${money(load.bookRequest?.ask ?? load.targetRate)} all in. Please reply to our email${ctx.carrier.inbound_key && inboundAddress(ctx.carrier.inbound_key) ? " or send the rate confirmation there" : ""}. Thanks.`;
